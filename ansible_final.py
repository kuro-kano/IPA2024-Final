...
def _run_playbook(
    ...
    try:
        ...
        inventory = _create_inventory(
            tmpdir_path, host_ip, ansible_user, ansible_pass, enable_pass
        )
        ...
        while True:
            try:
                ...
                return (
                    None
                    if operation_name == "showrun"
                    else f"Error: Cannot configure {operation_name} (checked via Ansible)"
                )

            except subprocess.TimeoutExpired:
                print(
                    f"[{operation_name}] ansible-playbook process timeout -> retry in 2s ..."
                )
                time.sleep(2)
                continue
                ...
                return (
                    None
                    if operation_name == "showrun"
                    else f"Error: Cannot connect to router {host_ip} - {str(e)}"
                )
